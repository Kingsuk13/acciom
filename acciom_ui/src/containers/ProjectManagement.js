...
import {
    ...
    addToProjectList
} from '../actions/projectManagementActions';
import {
    PROJECTS,
    ORGANIZATION,
    PROJECTNAME,
...
class ProjectManagement extends Component {
    ...
    deleteItemHandler = deleteProjectId => {
        this.setState({
            ...
            deleteConnectionID: deleteProjectId
        });
    };
    ...
    editHandler = index => {
        ...
        this.setState({
            projectNameInitialValue: localProjectList[index].project_name
        });
        this.setState({
            projectDescription: localProjectList[index].project_description
        });
        this.setState({
            projectDescriptionInitialValue:
                localProjectList[index].project_description
        });
    };
    ...
    saveDataHandler = index => {
        ...
        upDateProjectDetails = {
            ...
            project_description:
                localProjectListHandler[index].project_description,
        ...
        } else if (localProjectListHandler[index].project_name.length == 0) {
            toast.error(PROJNAMETEXT);
        }
        if (
            projectNameInitialValue === projectName &&
            projectDescriptionInitialValue === projectDescription
        ) {
            this.setState({ editIdx: -1 });
        }

        if (
            (localProjectListHandler[index].project_name.length > 0 &&
                projectNameInitialValue !== projectName) ||
            (localProjectListHandler[index].project_description.length > 0 &&
                projectDescriptionInitialValue !== projectDescription)
        ) {
    ...
    clearDataHandler = index => {
        ...
        if (clearProjectListHandler[index].project_description.length == 0) {
            clearProjectListHandler[
                index
            ].project_description = this.state.projectDescriptionInitialValue;
            this.setState({
                organizationDescription:
                    clearProjectListHandler[index].project_description
            });
        }
        if (clearProjectListHandler[index].project_name.length == 0) {
            clearProjectListHandler[
                index
            ].project_name = this.state.projectNameInitialValue;
            this.setState({
                organizationName: clearProjectListHandler[index].project_name
            });
        }
                    ...
                    action: (
                        <Fragment>
                            ...
                            <DeleteIcon
                                ...
                                style={{ color: '#696969', marginRight: '8px' }}
                                onClick={() => {
                                    this.deleteItemHandler(project.project_id);
                                }}
                            />
                        </Fragment>
                    ),
